...
// Wordly brand blue (matches the Figma `actions/primary` / brand mark fill).
const BRAND_FILL = "#017CFF";

// TODO(figma): confirm the exact tagline copy + lockup against the Logo node in
// the Wordly Design System file. Placeholder until the brand frame is shared.
const DEFAULT_TAGLINE = "AI translation for meetings & events";

export interface LogoProps extends React.HTMLAttributes<HTMLDivElement> {
  /** "color" = brand blue mark; "mono" = inherits currentColor (for dark/photo backgrounds). */
  variant?: "color" | "mono";
  /** Render the wordmark with a tagline lockup beneath it. */
  withTagline?: boolean;
  /** Tagline copy when `withTagline` is set. */
  tagline?: string;
  /** Class applied to the wordmark SVG (set height here, e.g. `h-6`). */
  markClassName?: string;
}

/** The Wordly wordmark on its own (no tagline). */
function Wordmark({
  variant = "color",
  className,
}: {
  ...
  const fill = variant === "mono" ? "currentColor" : BRAND_FILL;
  return (
    <svg
      viewBox="0 0 344 103"
      fill="none"
...
/**
 * Wordly logo. Use `withTagline` for the stacked lockup.
 *
 *   <Logo />                          // wordmark, brand blue
 *   <Logo variant="mono" className="text-white" />
 *   <Logo withTagline />              // wordmark + tagline
 */
    ...
    if (!withTagline) {
      return (
        <div ref={ref} className={cn("inline-flex", className)} {...props}>
          <Wordmark variant={variant} className={markClassName} />
        </div>
      );
    }
    return (
      ...
      >
        ...
        <span
          className={cn(
            ...
            variant === "mono" ? "text-current opacity-80" : "text-gray-600"
          )}
        >
          {tagline}
        </span>
